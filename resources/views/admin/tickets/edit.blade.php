...
@section('content')
  <section class="mt-5">
    ...
    <form action="{{ route('admin.tickets.update', ['id' => $ticket->getId()]) }}" method="POST" class="w-50">
      @csrf
      @method('PUT')
      ...
      <div id="ticket-plans" class="px-4 mt-5">
        ...
        @foreach($ticket->getPlans() as $key => $plan)
          <div class="mb-3 plan-item">
            <input type="hidden" name="ticket_plans[{{ $key }}][id]" value="{{ $plan->getId() }}">
            <div class="form-group mb-3">
      ...
      <button class="btn btn-primary"><i class="fa fa-paper-plane me-2"></i>Update Ticket</button>
    </form>
  </section>
